...
/** Various common methods used by the Mantis based runner. */
@SuppressWarnings({
...
public class Utils {
  ...
  public static Map.Entry<TupleTag<?>, PCollection<?>> getOutput(
      AppliedPTransform<?, ?, ?> appliedTransform) {
    return Iterables.getOnlyElement(
        Objects.requireNonNull(getOutputs(appliedTransform)).entrySet());
  }
      ...
      if (DoFnSignatures.requiresTimeSortedInput(doFn)) {
        throw new UnsupportedOperationException(
            String.format(
                ...
                MantisRunner.class.getSimpleName()));
      }
      return doFn;
    } catch (IOException e) {
      throw new RuntimeException(e);
  ...
  private static RuntimeException rethrow(IOException e) {
    // TODO(hmittal): provide a better implementation here!
    throw new RuntimeException(e);
  }
